...
$( function(){
    ...
    test( 'Data attribute options', function(){
        ...
        strictEqual( $( '#fileupload' ).fileupload( 'option', 'url' ), 'http://example.org' );
    } );

    test( 'File input initialization', function(){
        var fu = $( '#fileupload' ).fileupload();
        ok( fu.fileupload( 'option', 'fileInput' ).length, 'File input field inside of the widget' );
        ok( fu.fileupload( 'option', 'fileInput' ).length, 'Widget element as file input field' );
    } );
    ...
    test( 'Event listeners initialization', function(){
        ...
        } ), fileInput = fu.fileupload( 'option', 'fileInput' ), dropZone = fu.fileupload( 'option', 'dropZone' ), pasteZone = fu.fileupload( 'option', 'pasteZone' );
    ...
    test( 'destroy', function(){
        ...
        }, fu = $( '#fileupload' ).fileupload( options ), fileInput = fu.fileupload( 'option', 'fileInput' ), dropZone = fu.fileupload( 'option', 'dropZone' ), pasteZone = fu.fileupload( 'option', 'pasteZone' );
    ...
    test( 'option', function(){
        ...
        strictEqual( fu.fileupload( 'option', 'dropZone' )[0], document.body, 'Allow a query string as parameter for the dropZone option' );
        fu.fileupload( 'option', 'dropZone', document );
        strictEqual( fu.fileupload( 'option', 'dropZone' )[0], document, 'Allow a document element as parameter for the dropZone option' );
        fu.fileupload( 'option', 'pasteZone', 'body' );
        strictEqual( fu.fileupload( 'option', 'pasteZone' )[0], document.body, 'Allow a query string as parameter for the pasteZone option' );
        fu.fileupload( 'option', 'pasteZone', document );
        strictEqual( fu.fileupload( 'option', 'pasteZone' )[0], document, 'Allow a document element as parameter for the pasteZone option' );
        fu.fileupload( 'option', 'fileInput', ':file' );
        strictEqual( fu.fileupload( 'option', 'fileInput' )[0], $( ':file' )[0], 'Allow a query string as parameter for the fileInput option' );
        fu.fileupload( 'option', 'fileInput', $( ':file' )[0] );
        strictEqual( fu.fileupload( 'option', 'fileInput' )[0], $( ':file' )[0], 'Allow a document element as parameter for the fileInput option' );
    ...
    asyncTest( 'add', function(){
        ...
        $( '#fileupload' ).fileupload( {
            add: function( e, data ){
                strictEqual( data.files[0].name, param.files[0].name, 'Triggers add callback' );
            }
        } ).fileupload( 'add', param ).fileupload( 'option', 'add',function( e, data ){
    ...
    asyncTest( 'always', function(){
        ...
        fu.fileupload( 'add', param ).fileupload( 'option', 'url', '404' ).fileupload( 'add', param );
    } );
    ...
    test( 'change', function(){
        var fu = $( '#fileupload' ).fileupload(), fuo = fu.data( 'blueimp-fileupload' ) || fu.data( 'fileupload' ), fileInput = fu.fileupload( 'option', 'fileInput' );
        expect( 2 );
        fu.fileupload( {
    ...
    test( 'paste', function(){
        var fu = $( '#fileupload' ).fileupload(), fuo = fu.data( 'blueimp-fileupload' ) || fu.data( 'fileupload' );
        expect( 1 );
        fu.fileupload( {
    ...
    test( 'paramName', function(){
        ...
        $( '#fileupload' ).fileupload( {
            paramName: null,
            send: function( e, data ){
                strictEqual( data.paramName[0], data.fileInput.prop( 'name' ), 'Takes paramName from file input field if not set' );
                return false;
            }
        } ).fileupload( 'send', param );
    } );
    ...
    test( 'url', function(){
        ...
        $( '#fileupload' ).fileupload( {
            url: null,
            send: function( e, data ){
                strictEqual( data.url, $( data.fileInput.prop( 'form' ) ).prop( 'action' ), 'Takes url from form action if not set' );
                return false;
            }
        } ).fileupload( 'send', param );
    } );
    ...
    test( 'type', function(){
        ...
        $( '#fileupload' ).fileupload( {
            type: null,
            send: function( e, data ){
                strictEqual( data.type, 'POST', 'Request type is "POST" if not set to "PUT"' );
                return false;
            }
        } ).fileupload( 'send', param );
        $( '#fileupload' ).fileupload( {
            type: 'PUT',
            send: function( e, data ){
                strictEqual( data.type, 'PUT', 'Request type is "PUT" if set to "PUT"' );
                return false;
            }
        } ).fileupload( 'send', param );
    } );
    ...
    test( 'replaceFileInput', function(){
        var fu = $( '#fileupload' ).fileupload(), fuo = fu.data( 'blueimp-fileupload' ) || fu.data( 'fileupload' ), fileInput = fu.fileupload( 'option', 'fileInput' ), fileInputElement = fileInput[0];
        expect( 2 );
        fu.fileupload( {
            replaceFileInput: false,
            change: function( e, data ){
                strictEqual( fu.fileupload( 'option', 'fileInput' )[0], fileInputElement, 'Keeps file input with replaceFileInput: false' );
            },
            add: $.noop
        } );
        fuo._onChange( {
        ...
        fu.fileupload( {
            ...
            change: function( e, data ){
                notStrictEqual( fu.fileupload( 'option', 'fileInput' )[0], fileInputElement, 'Replaces file input with replaceFileInput: true' );
            },
            add: $.noop
        } );
        fuo._onChange( {
    ...
    asyncTest( 'forceIframeTransport', function(){
        ...
        $( '#fileupload' ).fileupload( {
            forceIframeTransport: true,
                ...
                strictEqual( data.dataType.substr( 0, 6 ), 'iframe', 'Iframe Transport is used' );
                start();
            }
        } ).fileupload( 'send', param );
    } );
    ...
    test( 'singleFileUploads', function(){
        expect( 3 );
        var fu = $( '#fileupload' ).fileupload(), param = {files: [
            {name: '1'},
            {name: '2'}
        ]}, index = 1;
        (fu.data( 'blueimp-fileupload' ) || fu.data( 'fileupload' ))._isXHRUpload = function(){
        ...
        } ).fileupload( 'add', param ).fileupload( 'option', 'singleFileUploads', false ).fileupload( 'add', param );
    } );
    ...
    asyncTest( 'sequentialUploads', function(){
        ...
        var param = {files: [
            ...
            {name: '5'},
            {name: '6'}
        ]}, addIndex = 0, sendIndex = 0, loadIndex = 0, fu = $( '#fileupload' ).fileupload( {
    ...
    if( $.support.xhrFileUpload ){
        asyncTest( 'multipart', function(){
            ...
            var param = {files: [
                ...
                multipart: false,
                always: function( e, data ){
                    strictEqual( data.contentType, param.files[0].type, 'non-multipart upload sets file type as contentType' );
                    strictEqual( data.headers['Content-Disposition'], 'attachment; filename="' + param.files[0].name + '"', 'non-multipart upload sets Content-Disposition header' );
                    start();
                }
            } );
            fu.fileupload( 'send', param );
    ...
    test( 'Widget initialization', function(){
        var fu = $( '#fileupload' ).fileupload();
        ok( fu.data( 'blueimp-fileupload' ) || fu.data( 'fileupload' ) );
        ok( $( '#fileupload' ).fileupload( 'option', 'uploadTemplate' ).length, 'Initialized upload template' );
        ok( $( '#fileupload' ).fileupload( 'option', 'downloadTemplate' ).length, 'Initialized download template' );
    } );
    ...
    test( 'Buttonbar event listeners', function(){
        ...
        ($( '#fileupload' ).data( 'blueimp-fileupload' ) || $( '#fileupload' ).data( 'fileupload' ))._renderDownload( files ).appendTo( $( '#fileupload .files' ) ).show().find( '.toggle' ).click();
    ...
    test( 'destroy', function(){
        ...
        } ).fileupload( 'add', {files: files} ).fileupload( 'destroy' );
    ...
    test( 'disable/enable', function(){
        ...
        strictEqual( buttonbar.find( 'input[type=file], button' ).not( ':disabled' ).length, 0, 'Disables the buttonbar buttons' );
        $( '#fileupload' ).fileupload( 'enable' );
        strictEqual( buttonbar.find( 'input[type=file], button' ).not( ':disabled' ).length, 4, 'Enables the buttonbar buttons' );
    } );
    ...
    test( 'destroy', function(){
        ...
        $( '#fileupload' ).fileupload( {
            destroy: function( e, data ){
                ...
                strictEqual( data.type, 'DELETE', 'Passes over deletion request type parameter' );
            }
        } );
        ($( '#fileupload' ).data( 'blueimp-fileupload' ) || $( '#fileupload' ).data( 'fileupload' ))._renderDownload( [
            {
                name: 'test',
                deleteUrl: 'test',
                deleteType: 'DELETE'
            }
        ] ).appendTo( $( '#fileupload .files' ) ).show().find( '.toggle' ).click();
    ...
    asyncTest( 'added', function(){
        ...
        $( '#fileupload' ).fileupload( {
            added: function( e, data ){
                start();
                strictEqual( data.files[0].name, param.files[0].name, 'Triggers added callback' );
            },
            send: function(){
                return false;
            }
    ...
    asyncTest( 'sent', function(){
        ...
        $( '#fileupload' ).fileupload( {
            sent: function( e, data ){
                start();
                strictEqual( data.files[0].name, param.files[0].name, 'Triggers sent callback' );
                return false;
            }
        } ).fileupload( 'send', param );
    } );
    ...
    test( 'autoUpload', function(){
        ...
        $( '#fileupload' ).fileupload( {
            ...
            send: function( e, data ){
                ...
                {name: 'test'}
            ]} ).fileupload( 'option', 'autoUpload', false ).fileupload( 'add', {files: [
                {name: 'test'}
            ]} );
    } );
    ...
    test( 'acceptFileTypes as HTML5 data attribute', function(){
        ...
        $( '#fileupload' ).attr( 'data-accept-file-types', regExp.toString() ).fileupload();
        strictEqual( $.type( $( '#fileupload' ).fileupload( 'option', 'acceptFileTypes' ) ), $.type( regExp ) );
        strictEqual( $( '#fileupload' ).fileupload( 'option', 'acceptFileTypes' ).toString(), regExp.toString() );
    } );
